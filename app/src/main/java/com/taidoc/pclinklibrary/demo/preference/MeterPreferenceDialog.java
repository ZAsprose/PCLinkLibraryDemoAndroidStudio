...
import static com.taidoc.pclinklibrary.demo.R.anim;
import static com.taidoc.pclinklibrary.demo.R.id;
import static com.taidoc.pclinklibrary.demo.R.layout;
import static com.taidoc.pclinklibrary.demo.R.string;
...
public class MeterPreferenceDialog extends FullScreenDialogFragmentBase implements OnSearchBLEClickListener {
    ...
    private Button.OnClickListener mSearchOnClickListener = new Button.OnClickListener() {
        ...
        public void onClick(View v) {
        	if (mSearch.getTag().toString().equals("0")) {
        		mSearch.setTag("1");
        		mSearch.setText(string.setting_meter_stop);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(layout.setting_meter, container, false);
        ...
        mProgress.setProgressDrawable(getResources().getDrawable(anim.progress));
        mProgress.setIndeterminateDrawable(getResources().getDrawable(anim.progress));
	...
	public void onBackPressed(LeavePageState state, Object param) {
		if (isModify()) {
    		showAlertDialog(state, getString(string.saveandleave), param);
    	}
    	else {
    		afterShowAlertDialog(state, string.no, param);
    	}
    }
	
	private void findViews(View view) {
    	mBack = (ImageButton) view.findViewById(id.btn_back);
    	mProgress = (ProgressBar) view.findViewById(id.pb_search);
    	mSearch = (Button) view.findViewById(id.btn_search);
    	mListView = (ListView) view.findViewById(id.lv_content);
    	mRoot = (RelativeLayout) view.findViewById(id.root);
    	mLVBackground = (LinearLayout) view.findViewById(id.lv_background);
    }
	...
	private void processScanDeviceTimeout() {
		...
		if (getActivity() != null) {
			if (mSearch.getTag().toString().equals("1")) {
				mSearch.setTag("0");
				mSearch.setText(string.meter_search);
	...
	private void showAlertDialog(final LeavePageState state, String message, final Object param) {
        ...
        builder.setNeutralButton(string.no, new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				backupData();
				afterShowAlertDialog(state, string.no, param);
	...
	private class ListAdapter extends BaseAdapter {
    	...
    	public View getView(final int position, View convertView, ViewGroup parent) {
    		...
    		View vi = convertView;
    		if(convertView == null) {
    			vi = inflater.inflate(layout.listview_content_delete, null);
    			viewHolder = new ViewHolder();
    			viewHolder.tvTitle = (AEditText) vi.findViewById(id.tv_title);
    			viewHolder.btnDel = (Button) vi.findViewById(id.btn_del);
	...
	private class LeAcceptThread extends Thread {
         ...
         * create a Bluetooth connection, will changed thread to
         */
        public void run() {
